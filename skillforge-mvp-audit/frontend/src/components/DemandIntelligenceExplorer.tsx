    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl text-slate-100 mb-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4 mb-6">
        ...
        <div className="text-xs text-slate-400 bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700/60 flex items-center gap-2">
          <span>Freshness: <strong className="text-slate-200 font-mono">2026-09-01</strong></span>
          <span>•</span>
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
            ...
            <select
              ...
              className="bg-slate-800 text-slate-200 text-xs px-3 py-1.5 rounded-lg border border-slate-700 focus:outline-none focus:border-cyan-500"
            >
          ...
          {loadingRankings ? (
            <div className="space-y-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-10 bg-slate-800/50 animate-pulse rounded-lg"></div>
              ))}
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-300">
                <thead className="bg-slate-800/70 text-slate-400 uppercase text-[11px]">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800">
                  {rankings.map((item, idx) => (
                    <tr key={item.skill_id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="py-2.5 px-3 font-mono text-slate-400">{idx + 1}</td>
                      <td className="py-2.5 px-3 font-medium text-white">{item.skill_name}</td>
                      ...
                      <td className="py-2.5 px-3 text-right font-mono text-slate-400">{item.role_count}</td>
      ...
      {activeTab === "compare" && (
        <div className="space-y-6">
          ...
          {loadingCompare ? (
            <div className="h-48 bg-slate-800/40 animate-pulse rounded-lg"></div>
          ) : comparisonData ? (
            <div className="space-y-6">
              {/* Summary Stats */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  <div className="text-[11px] text-slate-400">Compared Roles</div>
                  <div className="text-xl font-bold font-mono text-white mt-0.5">
                    {comparisonData.comparison_summary.compared_roles_count}
                  </div>
                </div>
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  <div className="text-[11px] text-slate-400">Total Unique Skills</div>
                  <div className="text-xl font-bold font-mono text-white mt-0.5">
                    {comparisonData.comparison_summary.total_unique_skills}
                  </div>
                </div>
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  <div className="text-[11px] text-slate-400">Shared Skills</div>
                ...
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  ...
                  <div className="text-xl font-bold font-mono text-slate-300 mt-0.5">India</div>
                </div>
              </div>
                  ...
                  <table className="w-full text-left text-xs text-slate-300">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {comparisonData.shared_skills.map((s) => (
                        <tr key={s.skill_id} className="hover:bg-slate-800/40">
                          <td className="py-2 px-3 font-medium text-white">{s.skill_name}</td>
                          <td className="py-2 px-3 text-slate-400">{s.category || "—"}</td>
                          <td className="py-2 px-3 text-right font-mono font-semibold text-cyan-300">
              ...
              <div className="space-y-3">
                <div className="text-xs font-semibold text-slate-300">Role-Specific Specializations</div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {comparisonData.roles.map((r) => {
                    const specific = comparisonData.role_specific_skills[r.role_id] || [];
                    return (
                      <div key={r.role_id} className="bg-slate-800/40 border border-slate-750 p-4 rounded-lg">
                        ...
                        {specific.length === 0 ? (
                          ...
                          <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                            {specific.map((sk) => (
                              <div
                                key={sk.skill_id}
                                className="flex justify-between items-center text-xs py-1 px-2 rounded bg-slate-800/60"
                              >
                                <span className="text-slate-200">{sk.skill_name}</span>
          ...
          {loadingSignals ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-800/40 border border-slate-800 p-4 rounded-lg">
                <div className="text-sm font-semibold text-white">{signalsData.role.title}</div>
                <div className="text-xs text-slate-400 mt-0.5">{signalsData.role.description}</div>
              </div>

              {/* Signals Overview Grid */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  <div className="text-[11px] text-slate-400">Total Skills</div>
                  <div className="text-xl font-bold font-mono text-white mt-0.5">
                    {signalsData.metrics.total_demanded_skills}
                  </div>
                </div>
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  <div className="text-[11px] text-slate-400">Avg Demand</div>
                  <div className="text-xl font-bold font-mono text-cyan-300 mt-0.5">
                    {Math.round(signalsData.metrics.average_demand_score * 100)}%
                  </div>
                </div>
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  <div className="text-[11px] text-slate-400">Avg YoY Growth</div>
                  <div className="text-xl font-bold font-mono text-teal-300 mt-0.5">
                    +{Math.round(signalsData.metrics.average_growth_rate * 100)}%
                  </div>
                </div>
                <div className="bg-slate-800/60 border border-slate-700/60 p-3 rounded-lg">
                  <div className="text-[11px] text-slate-400">Rising Skills</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-800/40 border border-slate-750 p-4 rounded-lg">
                  <div className="text-xs font-semibold text-slate-300 mb-3">Top 5 Demanded Skills</div>
                  <div className="space-y-2">
                    {signalsData.top_demanded_skills.map((s, idx) => (
                      <div key={s.skill_id} className="flex justify-between items-center text-xs py-1.5 px-2.5 rounded bg-slate-800/60">
                        <span className="font-medium text-white">{idx + 1}. {s.skill_name}</span>
                        <div className="flex items-center gap-3 font-mono">
                          <span className="text-slate-400">+{Math.round(s.growth_rate * 100)}%</span>
                          <span className="text-cyan-300 font-semibold">{Math.round(s.demand_score * 100)}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-slate-800/40 border border-slate-750 p-4 rounded-lg">
                  <div className="text-xs font-semibold text-slate-300 mb-3">Top 5 Fastest Growing Skills</div>
                  <div className="space-y-2">
                    {signalsData.fastest_growing_skills.map((s, idx) => (
                      <div key={s.skill_id} className="flex justify-between items-center text-xs py-1.5 px-2.5 rounded bg-slate-800/60">
                        <span className="font-medium text-white">{idx + 1}. {s.skill_name}</span>
                        <div className="flex items-center gap-3 font-mono">
                          <span className="text-emerald-400 font-semibold">+{Math.round(s.growth_rate * 100)}% YoY</span>
                          <span className="text-slate-400">{Math.round(s.demand_score * 100)}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {loadingTrends ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-300">
                ...
                <tbody className="divide-y divide-slate-800">
                  {trends.map((item) => (
                    <tr key={`${item.skill_id}-${item.role_id}`} className="hover:bg-slate-800/40 transition-colors">
                      <td className="py-2.5 px-3 font-medium text-white">{item.skill_name}</td>
                      <td className="py-2.5 px-3 text-slate-400">{item.role_title}</td>
