...
import { Sparkles, Map, AlertCircle, RefreshCw, X } from "lucide-react";
...
export default function SkillRoadmap({ targetRoleId, userId: controlledUserId }: SkillRoadmapProps) {
  ...
  useEffect(() => {
    ...
    async function runFetch() {
      setLoadingDetail(true);
      setError(null);
      const res = await fetchRoadmapDetail(selectedRoadmapId, {
        ordering,
        userId: effectiveUserId,
      });
      if (ignore) return;
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-white tracking-tight">
              Curated Skill Learning Roadmap
            </h2>
          ...
          <p className="text-xs text-neutral-400 max-w-2xl leading-relaxed">
            Explore structured, prerequisite-sequenced learning paths across 12 engineering domains
            curated with official documentation, vetted video courses, and interactive practice problems.
          </p>
        </div>
