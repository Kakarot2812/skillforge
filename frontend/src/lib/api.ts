...
// -----------------------------------------------------------------------------
// Curated Technology Skill Roadmap Catalog (Additive Feature)
// -----------------------------------------------------------------------------
...
export const fetchRoadmaps = fetchRoadmapsCatalog;
...
export async function fetchRoadmapDetail(
  ...
  try {
    ...
    const candidateId = options?.userId || getCandidateUserId();
    if (candidateId) params.append("user_id", candidateId);
...
export async function fetchRoadmapSkillDetail(
  ...
  try {
    const candidateId = userId || getCandidateUserId();
    const headers: Record<string, string> = { "Content-Type": "application/json" };
    if (candidateId) {
      headers["X-User-Id"] = candidateId;
    }
    const params = candidateId ? `?user_id=${encodeURIComponent(candidateId)}` : "";
...
export const fetchRoadmapSkill = fetchRoadmapSkillDetail;
...
export async function updateSkillProgress(
  ...
  try {
    let candidateId: string | null = userId || getCandidateUserId();
    if (!candidateId && typeof window !== "undefined") {
      candidateId = await ensureCandidateIdentity();
    }
    const headers: Record<string, string> = { "Content-Type": "application/json" };
    if (candidateId) {
      headers["X-User-Id"] = candidateId;
...
export const updateRoadmapSkillProgress = updateSkillProgress;
