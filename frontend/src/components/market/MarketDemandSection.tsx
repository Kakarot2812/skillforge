...
export default function MarketDemandSection({
  ...
  return (
    <div id="industry-market-demand-section" className="scroll-mt-20 space-y-6">
      {/* Container Card */}
      <div className="editorial-card p-6 sm:p-8 relative select-none transition-colors duration-200 space-y-6">
        {/* Section Header */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-border">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="editorial-btn-secondary !p-2 !rounded-md transition-colors cursor-pointer disabled:opacity-50"
              title="Refresh market demand data"
              aria-label="Refresh market data"
            >
              <RefreshCw className={`h-3.5 w-3.5 ${loadingDemand ? "animate-spin" : ""}`} />
            </button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          {loadingRoles ? (
            <div className="animate-pulse flex gap-2 overflow-x-auto pb-1">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-9 bg-surface-subtle rounded-md w-36 shrink-0 border border-border" />
              ))}
            </div>
          ) : (
              ...
              {roles.map((r) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-mono transition-all duration-150 cursor-pointer border ${
                      isSelected
                        ? "bg-foreground text-background border-foreground font-semibold"
                        : "bg-surface text-muted border-border hover:text-foreground hover:border-foreground"
                    }`}
                  >
                    {r.title}
                  </button>
        ...
        {selectedRole && (
          <div className="space-y-4">
            ...
            <div className="bg-surface-subtle border border-border rounded-md p-4 space-y-1">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-foreground">{selectedRole.title}</h3>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-sm bg-surface text-muted border border-border">
            ...
            {/* SQL-Derived Aggregate Cards (Large Numbers + Thin Dividers) */}
            <div className="border-y border-border py-4 my-2">
              <div className="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-border">
                ...
                <div className="p-3 text-center space-y-0.5">
                  ...
                  <span className="text-xl sm:text-2xl font-mono font-bold text-accent block truncate">
                    {demandMeta?.highest_demand_score !== undefined
                      ? `${Math.round(demandMeta.highest_demand_score * 100)}%`
                      : "—"}
                    {demandMeta?.top_skill && (
                      <span className="text-xs font-normal text-muted ml-1 font-sans">
                        ({demandMeta.top_skill})
                      </span>
                    )}
                  </span>
                ...
                <div className="p-3 text-center space-y-0.5">
                  ...
                  <span className="text-xl sm:text-2xl font-mono font-bold text-accent block">
        ...
        {/* Market vs. Candidate Boundary Alert */}
        <div className="p-3.5 rounded-md bg-surface-subtle border border-border text-xs text-muted space-y-1">
          <div className="flex items-center gap-1.5 text-foreground font-semibold">
            <BarChart3 className="h-3.5 w-3.5 text-accent" />
            <span>Market Demand Boundary Invariant:</span>
          </div>
          <p className="leading-relaxed">
            Market demand metrics reflect empirical hiring statistics across employer postings. High market demand indicates what employers are recruiting for, <strong className="text-foreground">not whether you possess or have verified the skill</strong>. Candidate possession and gaps are evaluated separately in Section 2 and Section 3.
          </p>
        </div>
        ...
        {error && (
          <div className="p-4 rounded-md bg-danger-subtle border border-danger/30 text-danger flex items-start gap-3 text-xs">
            ...
            <div className="flex-1">
              <strong className="font-medium">Market Data Notice:</strong>
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 pt-2">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-md bg-surface border border-border text-foreground placeholder-muted text-xs focus:outline-none focus:border-accent transition-all"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {/* Trend Filter Tabs */}
            <div className="inline-flex rounded-md bg-surface-subtle p-1 border border-border text-xs">
            ...
            <div className="relative inline-flex items-center">
              ...
              <select
                ...
                className="pl-3 pr-8 py-2 rounded-md bg-surface border border-border text-foreground text-xs focus:outline-none focus:border-accent cursor-pointer appearance-none font-mono"
              >
                <option value="DEMAND_DESC">Sort: Highest Demand</option>
                <option value="GROWTH_DESC">Sort: Fastest Growing</option>
                <option value="NAME_ASC">Sort: Name (A-Z)</option>
              </select>
              <ArrowUpDown className="h-3.5 w-3.5 absolute right-2.5 top-1/2 -translate-y-1/2 text-muted pointer-events-none" />
            </div>
          </div>
        </div>
        ...
        {loadingDemand && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="p-5 rounded-md bg-surface-subtle border border-border animate-pulse space-y-3"
              >
        ...
        {!loadingDemand && filteredSkills.length === 0 && (
          <div className="py-12 text-center space-y-3 max-w-md mx-auto border border-border border-dashed rounded-md bg-surface-subtle p-6">
            <div className="h-9 w-9 rounded-md bg-surface border border-border text-muted flex items-center justify-center mx-auto">
              <Filter className="h-4 w-4" />
            </div>
            <h4 className="text-sm font-bold text-foreground">No Skills Found</h4>
        ...
        {!loadingDemand && filteredSkills.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-muted px-1">
              <span>
                Displaying <strong className="text-foreground font-mono">{filteredSkills.length}</strong> Demanded Skills
              </span>
              <span className="text-[11px] font-mono text-muted">
